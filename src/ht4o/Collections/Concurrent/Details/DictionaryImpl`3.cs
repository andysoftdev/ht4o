...
namespace Hypertable.Persistence.Collections.Concurrent.Details
{
        ...
        where TComparer : struct, IEqualityComparer<TKey>
    {
        ...
        internal DictionaryImpl<TKey, TKeyStore, TValue, TComparer> _newTable;

        protected readonly ConcurrentDictionary<TKey, TValue, TComparer> _topDict;
        ...
        // read it yet.
        internal DictionaryImpl<TKey, TKeyStore, TValue, TComparer> CopySlotAndGetNewTable(int idx, bool shouldHelp)
        {
...
#endif
            var MIN_COPY_WORK = Math.Min(toCopy, CHUNK_SIZE); // Limit per-thread work
            ...
            while (_copyDone < toCopy)
            {
                ...
                if (!panic)
                {
                    ...
                    for (;;)
                    {
                        ...
                        var alreadyClaimed =
                            Interlocked.CompareExchange(ref _claimedChunk, claimedChunk + 1, claimedChunk);
                ...
                var workdone = 0;
                var copyStart = claimedChunk * CHUNK_SIZE;
                for (var i = 0; i < MIN_COPY_WORK; i++)
                {
                    if (_copyDone >= toCopy)
                    {
                        PromoteNewTable();
        ...
        protected abstract DictionaryImpl<TKey, TKeyStore, TValue, TComparer> CreateNew(int capacity);
        ...
        protected bool keyEqual(TKey key, TKeyStore entryKey)
        {
            ...
            return _keyComparer.Equals(key, other);
        }
        ...
        private bool PutSlotCopy(TKeyStore key, object value, int fullHash)
        {
            ...
            if (newTable != null)
            {
                var newTable1 = curTable.CopySlotAndGetNewTable(idx, false);
                Debug.Assert(newTable == newTable1);
                curTable = newTable;
                goto TRY_WITH_NEW_TABLE;
            }
        ...
        private DictionaryImpl<TKey, TKeyStore, TValue, TComparer> Resize()
        {
        ...
        private DictionaryImpl<TKey, TKeyStore, TValue, TComparer> ResizeImpl()
        {
            // No copy in-progress, so start one.  
            //First up: compute new table size.
            var oldlen = _entries.Length;

            const int MAX_SIZE = 1 << 30;
            const int MAX_CHURN_SIZE = 1 << 15;

            // First size estimate is roughly inverse of ProbeLimit
            var sz = Size + (MIN_SIZE >> REPROBE_LIMIT_SHIFT);
            var newsz = sz < MAX_SIZE >> REPROBE_LIMIT_SHIFT ? sz << REPROBE_LIMIT_SHIFT : sz;
            ...
            if (newsz <= oldlen)
            {
                ...
                if (resizeSpan < RESIZE_MILLIS_TARGET)
                    ...
                    newsz = Math.Max(newsz, (int) (oldlen * RESIZE_MILLIS_TARGET / resizeSpan));
            }
            ...
            var kBs4 = (((newsz << 1) + 4) << 3 /*word to bytes*/) >> 12 /*kBs4*/;
