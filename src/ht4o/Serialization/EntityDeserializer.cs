...
namespace Hypertable.Persistence.Serialization
{
    ...
    internal sealed class EntityDeserializer : Deserializer
    {
        ...
        /// The deserializing entity delegate.
        /// </param>
        private EntityDeserializer(EntityReader entityReader, BinaryReader binaryReader, DeserializingEntity deserializingEntity)
            : base(binaryReader)
        {
        ...
        /// The target object.
        /// </param>
        internal override void BeforeDeserializeObjectProperties(Type destinationType, Inspector inspector, object target)
        {
            var entityReference = inspector != null ? this.entityBindingContext.EntityReferenceForInspector(inspector) : null;
        ...
        /// The count.
        /// </param>
        internal override void ReadEnumerableItem(Type elementType, Tags tag, Inspector inspector, object collection, int pos, int count)
        {
            ...
            if (entitySpec != null)
            {
                ...
                if (inspector.Enumerable.HasIndexer)
                {
                    ...
                    entityScanTarget = new EntityIndexerScanTarget(elementType, entitySpec, inspector.Enumerable, collection, pos);
                }
                else
                {
                    entityScanTarget = new EntityCollectionScanTarget(elementType, entitySpec, inspector.Enumerable, collection);
                }
        ...
        /// The tag.
        /// </param>
        internal override void SetObjectProperty(InspectedProperty inspectedProperty, object target, object value, Tags tag)
        {
        ...
        protected override void DeferredReadObject<T>(Tags tag, Type destinationType, object any, Action<T> action)
        {
            ...
            if (entitySpec != null)
            {
                var entityScanTarget = new EntityScanTarget(destinationType, entitySpec, (target, value) => action((T)value));
                if (IsEntityReference(tag))
                {
                    this.entityScanner.Add(entityScanTarget);
                    this.ObjectRefs.Add(entityScanTarget);
            ...
            switch (tag)
            {
                case Tags.EntityRef:
                    entitySpec = new EntitySpec(
                        this.ReadType(), this.ReadString(), this.ReadString(), new Key(this.BinaryReader.ReadString(), this.ReadString(), this.ReadString()));
                    break;
                ...
                case Tags.EntityKey:
                    {
                        ...
                        if (entityReference == null)
                        {
                            throw new PersistenceException(string.Format(CultureInfo.InvariantCulture, @"{0} is not a valid entity", type));
                        }

                        var tableBinding = entityReference.TableBinding;
                        if (tableBinding == null)
                        {
                            throw new PersistenceException(string.Format(CultureInfo.InvariantCulture, @"Undefined table binding for type {0}", type));
                        }

                        entitySpec = new EntitySpec(
                            type, tableBinding.Namespace, tableBinding.TableName, new Key(this.BinaryReader.ReadString(), this.ReadString(), this.ReadString()));
                        break;
                    }
                ...
                case Tags.EntityRow:
                    {
                        ...
                        {
                            throw new PersistenceException(string.Format(CultureInfo.InvariantCulture, @"Undefined column binding for type {0}", type));
                        }

                        entitySpec = new EntitySpec(
                            type, tableBinding.Namespace, tableBinding.TableName, new Key(this.BinaryReader.ReadString(), columnBinding.ColumnFamily, columnBinding.ColumnQualifier));
                        break;
                    }
            }
