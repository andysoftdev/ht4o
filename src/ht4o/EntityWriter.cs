...
namespace Hypertable.Persistence
{
    ...
    internal sealed class EntityWriter
    {
        ...
        /// </exception>
        private EntityWriter(EntityContext entityContext, Type entityType, object entity, Behaviors behaviors, IdentitySet entitiesWritten)
        {
            if (behaviors.IsCreateLazy())
            {
                if (behaviors.DoNotCache())
                {
                    throw new ArgumentException(@"DontCache cannot be combined with CreateLazy", "behaviors");
                }

                if (behaviors.BypassWriteCache())
                {
                    throw new ArgumentException(@"BypassWriteCache cannot be combined with CreateLazy", "behaviors");
                }
            }
        ...
        /// The entity key.
        /// </returns>
        private static Key Persist(EntityContext entityContext, Type entityType, object entity, Behaviors behaviors, IdentitySet entitiesWritten)
        {
        ...
        private Key Persist()
        {
            ...
            {
                var value = EntitySerializer.Serialize(this.entityContext, this.entityType, this.entity, SerializationBase.DefaultCapacity, this.SerializingEntity);
                if (this.entityReference == null)
                {
                    throw new PersistenceException(string.Format(CultureInfo.InvariantCulture, @"{0} is not a valid entity", this.entityType));
                }
                ...
                if (dontCache || this.entitySpecsWritten.Add(entitySpec) || this.behaviors.BypassWriteCache())
                {
                    if (bypassEntitySpecsFetched || !this.entitySpecsFetched.Contains(entitySpec))
                    {
                        ...
                        if (this.tableMutatorType != type)
                        {
                            ...
                            this.tableMutator = this.entityContext.GetTableMutator(this.entityReference.Namespace, this.entityReference.TableName);
                        }
